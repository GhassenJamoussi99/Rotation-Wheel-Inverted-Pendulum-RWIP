...
//regulator
extern void computeStabilization(float currentPendulumVelocity);
extern void computeSwingUp(float currentPendulumVelocity);

//encoder
extern float getMecSpeed(MCI_Handle_t *pHandle);
extern float getAngle(MCI_Handle_t *pHandle);

//error_handling
extern char *getMotorStatus(MCI_Handle_t *pHandle);
extern char *faultCodeToString(uint16_t fCode);
extern int getCurrentMotorState(MCI_Handle_t *pHandle);

//led
extern void indicateError();
extern void indicateWarning();
...
// Assign the stabilization and swing-up control functions
void initRegulator(Regulator *regulator)
{
...
void initErrorHandling(ErrorHandling *errorHandling)
{
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
		// printf("STM state 3: %d\r\n", MCI_GetSTMState(pHandle));

		//ENCODER_M1._Super.hMecAngle =
	  // printf("wMecAngle %d = \r\n",(wMecAngle / RADTOS1);

		currentPositionRad = ((float)motorDegrees/180) * M_PI;
    //printf("currentPositionRad %f\r\n",currentPositionRad);
		man_mecAngle = RADTOS16 * currentPositionRad;

		//printf("motorDegrees %d\r\n",motorDegrees);
		//printf("man_mecAngle %d\r\n",man_mecAngle);
   ...
   // HAL_Delay(2000);
    ...
    //pHandle->pFOCVars->hTeref = 5000;

    /* USER CODE BEGIN 3 */
		//x=x+0.175;
		//MC_ProgramPositionCommandMotor1(x,0.1);
		//printf("Get occured faults when state = fault over %d\r\n", MCI_GetOccurredFaults(pHandle));

		//MC_ProgramPositionCommandMotor1(-3.14/2,0.1);
		//HAL_Delay(2000);
		//MC_ProgramTorqueRampMotor1(8000,0);
		//HAL_Delay(2000);

		//errorHandling.fault_code = errorHandling.getMotorStatus(pHandle);
		//printf("Current motor status = %s\r\n", errorHandling.fault_code);

		//encoder.pendulum_angle = encoder.getAngle(pHandle);
		//printf("Current pendulum position in degrees %.2f°\r\n", encoder.pendulum_angle);

		/*
		if (encoder.pendulum_angle >= 150 && encoder.pendulum_angle <= 210)
		{
		  regulator.computeStabilization(test);
		} else {
		  regulator.computeSwingUp(test);
		}
		*/
		//fflush(stdout);
		//encoder.pendulum_speed = encoder.getMecSpeed(pHandle);
		//printf("Current pendulum speed in rpm %f \r\n", encoder.pendulum_speed);
  }
  /* USER CODE END 3 */
}
