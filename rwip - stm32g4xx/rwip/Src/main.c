...
// regulator
extern void computeStabilization();
extern void computeSwingUp();

// encoder
extern float getMecSpeed(ENCODER_Handle_t *pHandle);
...
extern float calculatePendulumVelocity();
extern float calculatePendulumAcceleration();
...
int speed_var = 0;
int delay_var = 0;
float g_pendulum_velocity = 0.0f;
int tickdiff = 0;
float Kr[3] = {635.8617,68.5572,1.5};
float neutral_angle = 180;
int start_stabilization = 0;
int swing_level = 0;

//PID regulator for the stabilization
volatile float controlSignalRPM = 0.0f;
int level = 0;

// These flags are used for swing up 
bool stop_algo1 = false;
bool stop_algo2 = false;
bool stop_both_algos = false;

//float pre_stabilize_limit = 130;
//float post_stabilize_limit = 230;
float pre_stabilize_limit = 105;
float post_stabilize_limit = 255;
...
/*
void RWIP_Log(const char *format, ...) {
    char buffer[256];
    va_list args;
...
int main(void)
{
  ...
  MX_NVIC_Init();
  /* USER CODE BEGIN 2 */
  // while( EAC_IsAligned(&EncAlignCtrlM1) == false){printf("Alignment in progress...\r\n");}
 ...
 //TODO : IMPROVE PID OF MOTOR and IMPROVE PID OF STABILIZE
  // MAYBE CONSIDER THE SWITCH of direction - speed of motor is a big factor
  // because the bigger it is the stronger the break would be.
  // Left is positiive
  // right is negative
  while (1)
  {
